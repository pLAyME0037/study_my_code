...
bool org_daily_update(sqlite3 *db, long long id, const char *invoice_date,
    ...
    const char *sql =
        "UPDATE OrganizationDailyInvoices SET invoice_date = ?, amount_in_riel = ?, amount_in_dollar = ? WHERE id = ?;";
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
        LOG_SQLITE3_ERROR(db);
        return_defer(false);
    }
...
bool org_daily_detail_insert(sqlite3 *db, long long daily_id, long long ill_id,
    ...
    if (currency && currency[0]) sqlite3_bind_text(stmt, 4, currency, -1, SQLITE_TRANSIENT);
    else sqlite3_bind_null(stmt, 4);
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        LOG_SQLITE3_ERROR(db);
        return_defer(false);
...
bool org_balance_insert(sqlite3 *db, long long organization_id, long long invoice_id,
    ...
    if (balance_amount && balance_amount[0]) sqlite3_bind_text(stmt, 3, balance_amount, -1, SQLITE_TRANSIENT);
    else sqlite3_bind_null(stmt, 3);
    if (balance && balance[0]) sqlite3_bind_text(stmt, 4, balance, -1, SQLITE_TRANSIENT);
    else sqlite3_bind_null(stmt, 4);
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        LOG_SQLITE3_ERROR(db);
        return_defer(false);
